#!/usr/bin/env bash
# Uninstaller for battery-saver.
#
# Usage:
#   bash ~/.local/share/battery-saver/uninstall.sh
#   # or non-interactive:
#   bash ~/.local/share/battery-saver/uninstall.sh --yes
...
# Avoid pulling the rug out from under ourselves if invoked from the install dir.
cd /

# 1. Restore Adaptive mode so the BIOS isn't left locked at 50/80.
